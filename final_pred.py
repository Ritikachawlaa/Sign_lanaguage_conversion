...
class Application:
    ...
    def __init__(self):
        ...
        self.panel.place(x=50, y=80, width=400, height=300)  # For video feed

        self.panel2 = tk.Label(self.root)  # Placeholder for processed image or other visual
        self.panel2.place(x=500, y=80, width=400, height=300)

        self.T1 = tk.Label(self.root, text="Character :", font=("Courier", 24, "bold"))
        self.T1.place(x=50, y=400)

        self.panel3 = tk.Label(self.root, text="", font=("Courier", 24))  # For character display
        self.panel3.place(x=250, y=400)

        self.T3 = tk.Label(self.root, text="Sentence :", font=("Courier", 24, "bold"))
        self.T3.place(x=50, y=460)

        self.panel5 = tk.Label(self.root, text="", font=("Courier", 24))  # For sentence display
        self.panel5.place(x=250, y=460)

        self.T4 = tk.Label(self.root, text="Suggestions :", fg="red", font=("Courier", 24, "bold"))
        self.T4.place(x=50, y=520)

        # Buttons for Suggestions
        self.b1 = tk.Button(self.root, text="", font=("Courier", 20))
        self.b1.place(x=250, y=520, width=150, height=50)

        self.b2 = tk.Button(self.root, text="", font=("Courier", 20))
        self.b2.place(x=450, y=520, width=150, height=50)

        self.b3 = tk.Button(self.root, text="", font=("Courier", 20))
        self.b3.place(x=650, y=520, width=150, height=50)

        self.b4 = tk.Button(self.root, text="", font=("Courier", 20))
        self.b4.place(x=850, y=520, width=150, height=50)

        # Additional Buttons
        self.speak = tk.Button(self.root, text="Speak", font=("Courier", 20), command=self.speak_fun)
        self.speak.place(x=250, y=600, width=150, height=50)

        self.clear = tk.Button(self.root, text="Clear", font=("Courier", 20), command=self.clear_fun)
        self.clear.place(x=450, y=600, width=150, height=50)
        # Variables
        self.str = ""
        self.ccc = 0
        self.word = ""
        ...
        self.str = ""  # To store the current word
        self.sentence = ""  # To store the entire sentence
        self.prev_char = ""  # Store the previous character to avoid repeated space/next detection
        self.word = ""  # To store the current word being formed
        self.ten_prev_char = [""] * 10  # To track the last 10 characters
        self.count = 0  # Counter for keeping track of character positions
    ...
    def video_loop(self):
        try:
            ...
            if cv2image.any:
                ...
                if not hands[0]:  # If no hand is detected
                    if self.current_symbol != "blank":  # Check if the current symbol was not already "blank"
                        self.current_symbol = "blank"  # Assign "blank" gesture
                        if not self.space_flag:  # Add space only if not already added
                            self.str += " "  # Add space to the sentence
                            self.space_flag = True  # Mark space as added
                            print(f"Added space to sentence: {self.str}")
                    self.panel3.config(text=self.current_symbol, font=("Courier", 30))  # Update GUI for "blank"
                    self.panel5.config(text=self.str.strip(), font=("Courier", 30), wraplength=1025)
    ...
    def check_sign_language(self, ch1):
        # If a space ("  ") is detected, handle it as word separation
        if ch1 == "  " and self.prev_char != "  ":
            self.str += " "  # Add space to current word (word separation)
        
        # If "next" is detected, consider the current word as complete
        if ch1 == "next" and self.prev_char != "next":
            self.sentence += self.str.strip() + " "  # Add the current word to the sentence
            self.str = ""  # Reset the current word after appending it to the sentence
        
        # If "Backspace" is detected, remove the last character from the current word
        if ch1 == "Backspace":
            self.str = self.str[:-1]  # Remove last character
        
        # If any valid character is detected, add it to the current word
        if ch1 != "next" and ch1 != "  " and ch1 != "Backspace":
            self.str += ch1  # Add recognized character to the current word

        # Update previous character and other tracking variables
        self.prev_char = ch1
        self.count += 1
        self.ten_prev_char[self.count % 10] = ch1

        # If a word is formed, suggest completions (using a dictionary, e.g., 'ddd')
        if len(self.str.strip()) != 0:
            word = self.str.strip()  # Get the current word formed
            self.word = word
            # Example of calling a dictionary for suggestions (assuming 'ddd' is a dictionary object):
            # ddd.check(word)  # Check the word against a dictionary
            # lenn = len(ddd.suggest(word))  # Get word suggestions
            # Handle suggestions logic here (if needed)

# Debug prints to check word and sentence formation
        print(f"Current Word: {self.str.strip()}")
        print(f"Current Sentence: {self.sentence.strip()}")
